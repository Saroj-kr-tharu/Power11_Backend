...
class WalletService extends Service {
  ...
  async joinContestWalletOperation(userId, amount, referenceId, idempotencyKey) {
   ...
   async winContestWalletOperation(userId, amount, referenceId, contestJoinFee, idempotencyKey) {
    const transaction = await sequelize.transaction();
    try {

      // 0 check the idempotancy 
        console.log("key => ", idempotencyKey, " userId => ", userId)
        const requestHash = crypto.createHash('sha256').update(JSON.stringify({userId, amount, referenceId})).digest('hex');
        let key = null; 
        key = await idempotancyKeyService.getByData({key: idempotencyKey});
      ...
      const newAvailableBalance = balance + amount + contestJoinFee ; 
      let newLockedBalance = wallet?.lockedBalance - contestJoinFee;
      console.log('amount => ', amount ,  " wallet =>  ", wallet?.balance, " contestJoinFee =>  ", contestJoinFee, " newAvailableBalance =>  ", newAvailableBalance, " newLockedBalance => ", newLockedBalance)
        if (newLockedBalance < 0) newLockedBalance = 0;
      ...
      // STEP 10: Rollback DB transaction
        await transaction.rollback();
        console.log('Something went wrong in service (winContestWalletOperation)', error);
        throw error;
    }
  }
